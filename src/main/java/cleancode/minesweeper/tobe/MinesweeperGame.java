...
public class MinesweeperGame {
    ...
    private static void actOnCell(String cellInput, String userActionInput) {
        ...
        if (doesUserChooseToPlantOpenCell(userActionInput)) {
    ...
    private static boolean doesUserChooseToPlantOpenCell(String userActionInput) {
    ...
    private static void initializeGame() {
        ...
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                ...
                BOARD[row][col].updateNearyByLandMineCount(count);
            }
        }
    }
    ...
    private static void open(int row, int col) {
        if (row < 0 || row >= 8 || col < 0 || col >= BOARD_COL_SIZE) {
            return;
        }
